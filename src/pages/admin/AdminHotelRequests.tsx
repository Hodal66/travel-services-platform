...
const AdminHotelRequests: React.FC = () => {
  ...
  const handleViewDetails = (request: HotelRequest) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold">Hotel Booking Requests</h1>
      </div>
      ...
      <div className="flex gap-4 flex-wrap">
        <div className="flex items-center gap-2 border px-3 py-2 rounded flex-1">
          ...
          <input
            type="text"
            placeholder="Search by user, email, hotel, or location"
            value={searchTerm}
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredRequests.map((request) => (
              <tr key={request.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{request.hotelName}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{request.roomType}</td>
...
export default AdminHotelRequests;
